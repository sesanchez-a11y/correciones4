...
namespace TutoriasDeClases.Controllers
{
    ...
    public async Task<IActionResult> Register([FromBody] RegistroModel model)
    {
        ...
        try
        {
            // Validar que el rol no sea Admin (prevenir auto-registro como admin)
            var rolNormalized = (model.Rol ?? "").ToLower().Trim();
            if (rolNormalized == "admin")
                return Forbid(); // No permitir auto-registro como Admin

            // Validar que sea Tutor o Alumno
            if (rolNormalized != "tutor" && rolNormalized != "alumno")
                return BadRequest(new { message = "El rol debe ser 'Tutor' o 'Alumno'." });
            ...
            newUser.Rol = rolNormalized == "tutor" ? "Tutor" : "Alumno";
            // Hash de contraseña usando BCrypt (recomendado) o similar en producción
        ...
        public async Task<IActionResult> Me()
        {
            ...
            var publicUser = new {
                ...
                especializacion = user.Especializacion,
                rol = user.Rol,
                notificaciones = user.Notificaciones?.Select(n => new { id = n.Id, mensaje = n.Mensaje, leida = n.Leida, fechaLeido = n.FechaLeido })
            };
        ...
        public async Task<IActionResult> DeleteNotification(string id)
        {
            ...
            try
            {
                await _usuarioRepo.RemoveNotificationAsync(user.Id, id);
                return Ok(new { message = "Notificación eliminada." });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Error al eliminar notificación.", details = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> MarkNotificationRead(string id)
        {
            ...
            try
            {
                await _usuarioRepo.MarkNotificationAsReadAsync(user.Id, id);
                return Ok(new { message = "Notificación marcada como leída." });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Error al marcar notificación como leída.", details = ex.Message });
            }
        }
